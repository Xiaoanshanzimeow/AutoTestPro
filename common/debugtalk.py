...
class DebugTalk:
    ...
    def get_extract_data(self, node_name, randoms=None) -> str:
        ...
        if randoms is not None and bool(re.compile(r'^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$').match(randoms)):
            #数字正则表达式，判断randoms是不是数字
            randoms = int(randoms) #转为整形
            data_value = { #根据不同的key取不同的value
                randoms: self.get_extract_order_data(data, randoms), #随机---减一，因为列表从0开始
                0: random.choice(data), #0---随机选取data里的一个
                -1: ','.join(data), #把列表里的所有元素用逗号拼接成一个字符串
                -2: ','.join(data).split(','), #先拼接成字符串，再用 split(',') 按逗号切回列表
            }
